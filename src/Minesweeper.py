...
def populate(screen, grid, count, blockX, blockY, limX,
             limY):  # blockX, blockY = tiles for first click. LimX,limY = grid boundaries
    while count > 0:
        randX = random.randint(1, limX) - 1
        randY = random.randint(1, limY) - 1
        if grid[randX][randY].isBomb == True or (
                blockX - 1 <= randX <= blockX + 1 and blockY - 1 <= randY <= blockY + 1):  # creating a 3x3 safezone for first click
            continue
        else:
...
def ui_drawRRect(screen, x, y, color, width, height, radius, outline=0):  # Draw a rounded rect
    ...
    pygame.draw.rect(screen, color, rectVert, outline)
    pygame.draw.rect(screen, color, rectHori, outline)
    pygame.draw.circle(screen, color, ((x + radius), (y + radius)), radius, outline)  # top left corner
    pygame.draw.circle(screen, color, ((x + (width - radius)), (y + radius)), radius, outline)  # top right corner
    pygame.draw.circle(screen, color, ((x + radius), (y + (height - radius))), radius, outline)  # bot left corner
    pygame.draw.circle(screen, color, ((x + (width - radius)), (y + (height - radius))), radius, outline)  # bot right corner
...
def gameOverDialogue(screen, res, score, remaining_bombs):
    ...
    text_to_screen(screen, "Score: ", body.centerx - body.width * 0.3 + 5, body.y + body.height * 0.3, size=20,
                   color=cc.RAINEE)
    text_to_screen(screen, int(score), body.centerx + body.width * 0.3 + 10, body.y + body.height * 0.3, size=20,
                   color=cc.RAINEE)

    button_reset = ui_drawRRect(screen, buttonX, body.y + int(body.height * 0.4), cc.MYRTLE, buttonWidth, buttonheight,
                                2)
    button_quit = ui_drawRRect(screen, buttonX, body.y + int(body.height * 0.8), cc.MYRTLE, buttonWidth, buttonheight,
                               2)
    button_Menu = ui_drawRRect(screen, buttonX, body.y + int(body.height * 0.6), cc.MYRTLE, buttonWidth, buttonheight,
                               2)
...
def game(res, screen, bomb_count, debug=False):
    ...
    while running:  # Main Loop
        ...
        if not GameOver:  # Freeze the processing if the player loses
            ...
            if pygame.mouse.get_pressed()[0] and not HUD.collidepoint(mousex, mousey) and tileHover.state != 'WARN':  # LMB Click handling
                pygame.time.wait(60)
            ...
            if button_menu.collidepoint(mousex, mousey) and pygame.mouse.get_pressed()[
                0]:  # Return back to the main menu on click
                pygame.time.wait(60)
                pList = []
                goMainMenu()
